---
title: "Comparative Analysis of Classification Models for Predicting Term Deposit Subscriptions"
author: "Group 14"
...
df_new <- cbind(df_not_numeric, df_numeric) # non-numeric data set 'df_not_numéric'
#Summary Statistics#
...
before actually applying LDA. First, classes are mutually exclusive and
...
class-covariance matrix. We would use LDA in case of equal covariance and
QDA in case of unequal covariance.
...
considered is closer to random guess, 0.59, hence providing evidence of a
...
each class to have its own covariance matrix in or to capture complex
decision boundaries.

QDA is actually a better choice for the bank marketing dataset as it
considers the case with unequal class covariance matrix which is the
case in the data being analysed in this study. So, in addition to LDA,
...
### Results
Results showed that tuning improves both Tree Bagging and Random Forest models slightly in terms of accuracy, specificity, and AUC. However, sensitivity remains relatively low for all models, indicating a challenge in correctly identifying positive cases. The Random Forest Tuned model shows the best overall balance of metrics, suggesting it as the best model among those evaluated.
